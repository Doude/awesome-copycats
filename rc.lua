...
run_once({ "~/.config/awesome/bin/xsettingsd-setup" }) -- setup xsettings
run_once({ "unclutter -root" }) -- entries must be comma-separated
run_once({ "nm-applet" }) -- entries must be comma-separated
...
local modkey       = "Mod4"
local altkey       = "Mod1"
local terminal     = awful.util.getdir("config") .. "/bin/urxvt"
...
local function screenshot(client)
    ...
    awful.spawn("import -quality 95 " .. client .. " " .. path, false)
end

-- {{{ Key bindings
globalkeys = awful.util.table.join(
    -- Take a screenshot
    awful.key({}, "XF86LaunchA", function() screenshot("root") end,
              {description = "take whole screenshot", group = "hotkeys"}),
    awful.key({ modkey }, "XF86LaunchA", function(c) screenshot(c) end,
              {description = "take focused windows screenshot", group = "hotkeys"}),
    -- awful.key({ modkey, "Shift" }, "XF86LaunchA", screenshot,
    --           {description = "select windows to screenshot", group = "hotkeys"}),

    -- Hotkeys
    awful.key({ modkey, }, "s",      hotkeys_popup.show_help,
              {description = "show help", group="awesome"}),
    awful.key({}, "XF86ScreenSaver", lock,
              {description = "Lock screen and screensaver", group="awesome"}),
    awful.key({ modkey, }, "Delete", lock,
              {description = "Lock screen and screensaver", group="awesome"}),
    awful.key({ modkey, }, "i", function (c) debug.client_info(c) end,
              {description = "Get client-related information", group="awesome"}),
    awful.key({ modkey }, "p", function() xrandr.xrandr() end,
              {description = "change screen setup", group = "awesome"}),
    ...
    -- Dynamic tagging
    awful.key({ modkey }, "a", function () lain.util.add_tag() end,
              {description = "add new tag", group = "tag"}),
    awful.key({ modkey, "Control" }, "a", function () lain.util.rename_tag() end,
              {description = "rename tag", group = "tag"}),
    -- awful.key({ modkey, "Shift" }, "a", function () lain.util.move_tag(-1) end,
    --           {description = "move tag to the left", group = "tag"}),
    -- awful.key({ modkey, "Shift" }, "Right", function () lain.util.move_tag(1) end,
    --           {description = "move tag to the right", group = "tag"}),
    awful.key({ modkey, "Shift" }, "a", function () lain.util.delete_tag() end,
    ...
    awful.key({ modkey }, "d", function () awful.spawn(guieditor) end,
...
awful.rules.rules = {
    ...
    -- Set Slack to always map on the 'chat' tag.
    { rule = { class = "Slack" },
    properties = { screen = 1, tag = awful.util.tagnames[3] } },

    -- Set Skype to always map on the 'chat' tag.
    { rule = { class = "Skype" },
    properties = { screen = 1, tag = awful.util.tagnames[3] } },
